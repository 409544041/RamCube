...
namespace ProjectDawn.LocalAvoidance.Demo
{
    ...
    public class BoidsSystem : System
    {
        ...
        void FixedUpdate()
        {
            ...
            foreach (var agent in agents)
            {
                ...
                if (agent.CanMove())
                {
                    ...
                    impulse += GetAvoidVision(nearbyAgents, agent, math.normalizesafe(agent.Destination - agent.Position)) * AvoidanceWeight;
        ...
        float2 GetAvoidVision(IList<BoidsAgent> agents, BoidsAgent agent,
            float2 desiredDirection)
        {
            ...
            {
                Velocity = new float3(agent.Velocity.x, 0, agent.Velocity.y),
                ForwardVelocityScaler = SonarForwardVelocityScaler,
                BackVelocityScaler = SonarBackVelocityScaler,
            };

            SonarAvoidance vision = new SonarAvoidance(
            ...
            vision.InsertObstacle(math.normalizesafe(-new float3(agent.Velocity.x, 0, agent.Velocity.y)), math.radians(135));

            vision.FindClosestDirection(out float3 newDirection);

            if (agent.gameObject.TryGetComponent(out AgentDebug agentDebug))
            {
                if (agentDebug.Vision.IsCreated)
                    agentDebug.Vision.Dispose();
                agentDebug.Vision = new SonarAvoidance(vision, Unity.Collections.Allocator.Persistent);
            }
