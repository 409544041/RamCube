...
namespace ProjectDawn.LocalAvoidance.Demo
{
    ...
    public class BoidsAgent : MonoBehaviour
    {
        ...
        {
            // For nice boids effect we do not avoid agents that move similar way
            return this != agent && !Similar(agent);
        }

        /// <summary>
        /// True if agent is moving in similar direction and in front of
        /// </summary>
        bool Similar(BoidsAgent agent)
        {
            var direction = math.normalizesafe(agent.Position - Position);
            bool agentIsInFront = math.dot(direction, math.normalizesafe(_Velocity)) >= 0.5f; // 60 degress
            bool agentHasSimilarDirection = math.dot(math.normalizesafe(agent.Velocity), math.normalizesafe(_Velocity)) >= 0.86f; // 30 degress
            return agentIsInFront && agentHasSimilarDirection;
        }
